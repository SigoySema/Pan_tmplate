...
class CrossMamba_(nn.Module):
    ...
    def forward(self, ms, pan):  # ms (10,7744,96), pan (10,7744,96)
        ...
        #ms = ms.reshape(batch_size, channels, -1)
...
if __name__ == "__main__":
    ...
    # 初始化模型
    dim = 24
    model = CrossMamba_(dim).to(device)

    # 创建虚拟输入数据
    batch_size = 5
    num_patches = 128*128
    channels = dim
    ms = torch.randn(batch_size, num_patches, channels).to(device)
    pan = torch.randn(batch_size, num_patches, channels).to(device)
